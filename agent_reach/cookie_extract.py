...
Extracts: Twitter/X cookies in one shot.
...
def configure_from_browser(browser: str, config) -> List[Tuple[str, bool, str]]:
    ...
    if "xueqiu" in extracted:
        ...
        if cookie_str and "xq_a_token" in cookie_str:
            ...
            n_cookies = len(cookie_str.split(";"))
            results_list.append(("Xueqiu", True, f"{n_cookies} cookies (xq_a_token present)"))
        elif cookie_str:
            results_list.append(("Xueqiu", False,
                                 f"Found {len(cookie_str.split(';'))} cookies but xq_a_token missing — "
                                 f"please log in to xueqiu.com in {browser} first"))
